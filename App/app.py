...
def hello_world():
    ...
    if request_type_str == 'GET':
        return render_template('index.html', href='static/Test.png')
    else:
...
def make_picture(training_data_filename, model, new_input_np_arr,output_file):
  ...
  fig.add_trace(go.Scatter(x=new_input_np_arr.reshape(len(new_input_np_arr)),y=new_preds, name='New Outputs', mode='markers', marker=dict(color='yellow',size=15,line=dict(color='black',width=3))))
